...
from app.schemas.info_schema import InfoCreate, InfoResponse, InfoUpdate
from app.routers.llm_router import llm_router, image_llm_router
...
# 정보검색 (전체 정보목록, 개별 정보 검색)
# 전체 정보 검색 (GET /info)
@app.get("/info", response_model=List[InfoResponse], tags=["정보"])
def get_info(
  ...
  정보 목록을 조회합니다.
  """
  items = list(info_db.values())
  
  if address : 
    items = [info for info in items if info["address"] == address]
  return items

# 개별도서 검색
@app.get("/info/{info_id}", response_model=InfoResponse, tags=["정보"])
def get_info(
  info_id:int = Path(
    ..., ge=1, description="정보 ID로 정보 검색"
  )
) : 
  """
  정보 ID로 정보 한건 조회
  """
  if info_id not in info_db :
    raise HTTPException(
      status_code=404,  # not found
      detail=f"회원 {info_id}번을 찾을 수 없습니다"
  )
  return info_db[info_id]


# 정보 수정 (PUT /info/{info_id})
@app.put("/info/{info_id}", response_model=InfoResponse, tags=["정보"])
def update_info(info_id:int, update:InfoUpdate) :
  """
  정보를 수정합니다.
  """
  if info_id not in info_db :
...
# 정보삭제 (DELETE /info/{info_id})
@app.delete("/info/{info_id}", status_code=204, tags=["정보"])
