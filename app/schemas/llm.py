# 파일 위치 : astapi_basic/app/schemas/llm.py
# 역할 : LLM API의 입출력 데이터 구조(json)만 정리합니다.
...
class SummarizeRequest(BaseModel) :
  ...
  max_length:int = Field(100, ge=10, le=500, description="요약 최대 길이(글자의)", 
                         deprecated="다음 버전에서 없어질 가능성이 있습니다. replace=....")
...
class SentimentReviewRequest(BaseModel):
  text: str = Field(..., min_length=1, description="감정 분석할 텍스트(리뷰)")
